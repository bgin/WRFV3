...
SUBROUTINE med_add_config_info_to_grid ( grid )
 ...
 grid % skebs                      = model_config_rec % skebs (grid%id)
 grid % stoch_force_opt            = model_config_rec % stoch_force_opt (grid%id)
 grid % skebs_vertstruc            = model_config_rec % skebs_vertstruc 
 ...
 grid % iseed_skebs                = model_config_rec % iseed_skebs 
 ...
 grid % sppt                       = model_config_rec % sppt (grid%id)
 grid % gridpt_stddev_sppt         = model_config_rec % gridpt_stddev_sppt (grid%id)
 grid % stddev_cutoff_sppt         = model_config_rec % stddev_cutoff_sppt (grid%id)
 grid % lengthscale_sppt           = model_config_rec % lengthscale_sppt (grid%id)
 grid % timescale_sppt             = model_config_rec % timescale_sppt (grid%id)
 grid % sppt_vertstruc             = model_config_rec % sppt_vertstruc 
 grid % iseed_sppt                 = model_config_rec % iseed_sppt 
 grid % rand_perturb               = model_config_rec % rand_perturb (grid%id)
 grid % gridpt_stddev_rand_pert    = model_config_rec % gridpt_stddev_rand_pert (grid%id)
 grid % stddev_cutoff_rand_pert    = model_config_rec % stddev_cutoff_rand_pert (grid%id)
 grid % lengthscale_rand_pert      = model_config_rec % lengthscale_rand_pert (grid%id)
 grid % timescale_rand_pert        = model_config_rec % timescale_rand_pert (grid%id)
 grid % rand_pert_vertstruc        = model_config_rec % rand_pert_vertstruc 
 grid % iseed_rand_pert            = model_config_rec % iseed_rand_pert 
 grid % skebs_on                   = model_config_rec % skebs_on 
 grid % sppt_on                    = model_config_rec % sppt_on 
 grid % rand_perturb_on            = model_config_rec % rand_perturb_on 
 grid % num_stoch_levels           = model_config_rec % num_stoch_levels 
 ...
 grid % extrap_below_grnd          = model_config_rec % extrap_below_grnd 
 grid % p_lev_missing              = model_config_rec % p_lev_missing 
 grid % p_lev_interval             = model_config_rec % p_lev_interval (grid%id)
 grid % z_lev_diags                = model_config_rec % z_lev_diags 
 grid % z_lev_diags_dfi            = model_config_rec % z_lev_diags_dfi 
 grid % num_z_levels               = model_config_rec % num_z_levels 
 grid % z_levels                   = model_config_rec % z_levels (grid%id)
 grid % z_lev_missing              = model_config_rec % z_lev_missing 
 grid % z_lev_interval             = model_config_rec % z_lev_interval (grid%id)
 ...
 grid % afwa_therm_opt             = model_config_rec % afwa_therm_opt (grid%id)
 grid % afwa_turb_opt              = model_config_rec % afwa_turb_opt (grid%id)
 grid % afwa_buoy_opt              = model_config_rec % afwa_buoy_opt (grid%id)
 grid % afwa_hailcast_opt          = model_config_rec % afwa_hailcast_opt (grid%id)
 grid % afwa_ptype_ccn_tmp         = model_config_rec % afwa_ptype_ccn_tmp 
 grid % afwa_ptype_tot_melt        = model_config_rec % afwa_ptype_tot_melt 
 grid % afwa_bad_data_check        = model_config_rec % afwa_bad_data_check 
 grid % nssl_ipelec                = model_config_rec % nssl_ipelec (grid%id)
 grid % nssl_isaund                = model_config_rec % nssl_isaund 
 grid % nssl_iscreen               = model_config_rec % nssl_iscreen 
 grid % nssl_lightrad              = model_config_rec % nssl_lightrad 
 grid % nssl_idischarge            = model_config_rec % nssl_idischarge 
 grid % nssl_ibrkd                 = model_config_rec % nssl_ibrkd 
 grid % nssl_ecrit                 = model_config_rec % nssl_ecrit 
 grid % nssl_disfrac               = model_config_rec % nssl_disfrac 
 grid % elec_physics               = model_config_rec % elec_physics 
 grid % perturb_bdy                = model_config_rec % perturb_bdy 
 grid % perturb_chem_bdy           = model_config_rec % perturb_chem_bdy 

...
SUBROUTINE med_nest_initial ( parent , nest , config_flags )
  ...
  ELSE  
     ...
     IF ( ierr .NE. 0 ) THEN
       ...
       CALL wrf_error_fatal3("mediation_integrate.b",659,&
message )
     ENDIF
...
SUBROUTINE med_hist_out ( grid , stream, config_flags )
   ...
   IF ( stream .LT. first_history .OR. stream .GT. last_auxhist ) THEN
     ...
     CALL wrf_error_fatal3("mediation_integrate.b",1686,&
message )
   ENDIF
...
write(0,*)"mediation_integrate.b",1724,trim(n2)
write(0,*)"mediation_integrate.b",1725,' grid%id ',grid%id,' grid%oid ',grid%oid
...
SUBROUTINE med_auxinput_in ( grid , stream, config_flags )
   ...
   IF ( stream .LT. first_auxinput .OR. stream .GT. last_auxinput ) THEN
     ...
     CALL wrf_error_fatal3("mediation_integrate.b",1779,&
message )
   ENDIF
...
SUBROUTINE med_filter_out ( grid , config_flags )
   ...
   END IF
     ...
     IF ( ierr .NE. 0 ) THEN
       CALL wrf_error_fatal3("mediation_integrate.b",1841,&
message )
     ENDIF

     IF ( ierr .NE. 0 ) THEN
       CALL wrf_error_fatal3("mediation_integrate.b",1846,&
message )
     ENDIF
       ...
       IF ( .NOT. lbc_opened ) THEN
         CALL construct_filename2a ( bdyname , config_flags%bdy_inname , grid%id , 2 , " " )
          ...
          IF ( ierr .NE. 0 ) THEN
            ...
            CALL wrf_error_fatal3("mediation_integrate.b",1955,&
message )
          ENDIF
       ELSE
       ...
       IF ( ierr .NE. 0 .and. ierr .NE. WRF_WARN_NETCDF ) THEN
         ...
         CALL wrf_error_fatal3("mediation_integrate.b",1984,&
message )
       ENDIF
...
SUBROUTINE open_aux_u ( grid , config_flags, stream, alarm_id, &
   ...
   IF ( stream .LT. first_stream .OR. stream .GT. last_stream ) THEN
     ...
     CALL wrf_error_fatal3("mediation_integrate.b",2094,&
message )
   ENDIF
...
SUBROUTINE open_hist_w ( grid , config_flags, stream, alarm_id, &
   ...
   IF ( stream .LT. first_history .OR. stream .GT. last_history ) THEN
     ...
     CALL wrf_error_fatal3("mediation_integrate.b",2166,&
message )
   ENDIF

...
wrf_err_message )
    ...
    IF ( ( switch .EQ.     input_only  ) .OR. &
       ...
       ierr = max( ierr, ierr3 )
       IF ( ierr3 .NE. 0 ) CALL wrf_error_fatal3("<stdin>",479,&
...
'Cannot find a valid time to start the LBC during this restart, likely ran out of time periods to test' )
                  END IF
                  CALL wrf_get_dom_td_char ( fid , 'THISBDYTIME' ,  current_date(1:19), this_datestr , ierr )
                  CALL wrf_get_dom_td_char ( fid , 'NEXTBDYTIME' ,  current_date(1:19), next_datestr , ierr )
                  CALL wrf_atotime( next_datestr(1:19), grid%next_bdy_time )
...
'Problems backing up in the LBC file to find startig location for restart' )
                  END IF
              END IF
          END IF

  ...
  SUBROUTINE  mym_condensation (kts,kte,  &
    ...
    DO k = kts,kte-1
       ...
       ELSE IF (bl_mynn_cloudpdf >= 2) THEN
          ...
          sgm(k) = MAX(1.e-10, 0.2*ls*SQRT(MAX(0., & 
                  & (a(k)*dqw/dzk)**2              & 
...
ENDIF
  ...
  SUBROUTINE GET_PBLH(KTS,KTE,zi,thetav1D,qke1D,zw1D,dz1D,landsea,kzi)
    ...
    REAL :: delt_thv   
    REAL, PARAMETER :: sbl_lim  = 400. 
    REAL, PARAMETER :: sbl_damp = 800.
  ...
  IF ( fltv > 0.002 .AND. PBLH > 100. .AND. superadiabatic) then
    ...
    do i=1,Nup
      do k=kts+1,kte
        ...
        ENTf(k,i)=MIN(ENTf(k,i),9.9) 
        ENTf(k,i)=MAX(ENTf(k,i),0.05) 
      enddo
    enddo
    ...
    DO I=1,NUP
       ...
       UPA(1,I)=0.2*ERF(wtv/(sqrt(2.)*sigmaW)) - 0.2*ERF(wlv/(sqrt(2.)*sigmaW))   
       ...
       k50=1 
       DO k=1,k50
         UPQT(1,I) = UPQT(1,I) +QT(k) +0.58*UPW(1,I)*sigmaQT/sigmaW 
         UPTHV(1,I)= UPTHV(1,I)+THV(k)+0.58*UPW(1,I)*sigmaTH/sigmaW 

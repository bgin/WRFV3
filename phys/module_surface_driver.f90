...
CONTAINS
   ...
   SUBROUTINE surface_driver(                                         &
     ...
     &          ,snowncv, anal_interval, lai, imperv, canfra          & 
     ...
     &          ,julian,julyr,drelr_urb2d,drelb_urb2d,drelg_urb2d     & 
   ...
   REAL, DIMENSION( ims:ime, jms:jme ), OPTIONAL, INTENT(INOUT)::   rhosnf 
   REAL, DIMENSION( ims:ime, jms:jme ), OPTIONAL, INTENT(OUT) ::    precipfr 
   REAL, DIMENSION( ims:ime, jms:jme ), OPTIONAL, INTENT(INOUT)::   snowfallac 
  ...
  real,    optional,  dimension(ims:ime,1:maxpatch,jms:jme ),intent(inout) ::  &
   ...
   REAL, OPTIONAL, DIMENSION( ims:ime, jms:jme ), INTENT(INOUT) :: CHC_SFCDIF
     ...
     INTEGER,  INTENT(IN), OPTIONAL :: julian,julyr                            
     ...
     REAL, OPTIONAL, DIMENSION( ims:ime , 1:num_soil_layers, jms:jme ), &       
           INTENT(INOUT)  :: TGRL_URB3D                                 
     REAL, OPTIONAL, DIMENSION( ims:ime , 1:num_soil_layers, jms:jme ), &       
           INTENT(INOUT)  :: SMR_URB3D                                 
     REAL, OPTIONAL, DIMENSION( ims:ime , 1:num_soil_layers, jms:jme ), &       
     ...
     IF(SF_URBAN_PHYSICS.eq.1) THEN
       DO j=j_start(ij),j_end(ij)                             
         DO i=i_start(ij),i_end(ij)                           
	      ...
	      IVGTYP(I,J) == LOW_DENSITY_RESIDENTIAL .or. & 
              IVGTYP(I,J) == HIGH_DENSITY_RESIDENTIAL .or.&
	      IVGTYP(I,J) == HIGH_INTENSITY_INDUSTRIAL) THEN  
...
'Lacking arguments for P-X LSM in surface driver')
     ...
     IF (MYJ) call wrf_error_fatal3("<stdin>",3142,&

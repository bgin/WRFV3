...
CONTAINS
   SUBROUTINE cumulus_driver(grid                                     &
                     ...
                     ,cldfra_dp,cldfra_sh,w_up                        & 
                     ,udr_kf,ddr_kf,uer_kf,der_kf,timec_kf,kf_edrates & 
                     ,QC_CU,QI_CU                                     &
...
'Lacking arguments for CU_NTIEDTKE in cumulus driver')
     ...
     CASE (NSASSCHEME)
        ...
        ELSE
          CALL wrf_error_fatal3("<stdin>",1314,&
'Lacking arguments for CU_NSAS in cumulus driver')
     ...
     CASE (KFCUPSCHEME)
        ...
        CALL KF_CUP_CPS( GRID_ID=grid%grid_id                        & 
             ,U=u ,V=v ,TH=th ,T=t ,W=w ,RHO=rho                &
             ,RAINCV=raincv,NCA=nca ,DZ8W=dz8w,XLAND=xland      & 
             ,PCPS=p, PI=pi ,W0AVG=W0AVG                        &
             ,CUTOP=HTOP,CUBOT=HBOT                             &
             ,XLV0=XLV0 ,XLV1=XLV1 ,XLS0=XLS0 ,XLS1=XLS1        &
             ,CP=CP ,R=R_d ,G=G ,EP1=EP_1 ,EP2=EP_2             &
             ...
             ,IMS=ims,IME=ime,JMS=jms,JME=jme,KMS=kms,KME=kme   &
             ,ITS=its,ITE=ite,JTS=jts,JTE=jte,KTS=kts,KTE=kte   &
             ,PSFC=psfc,Z=z,Z_AT_W=z_at_w,HT=ht,TSK=tsk         & 
             ,HFX=hfx,QFX=qfx                                   & 
             ,MAVAIL=mavail,SF_SFCLAY_PHYSICS=sf_sfclay_physics & 
             ,BR=br,REGIME=regime,PBLH=pblh,KPBL=kpbl           & 
             ,T2=t2,Q2=q2                                       & 
             ,SLOPESFC=slopeSfc,SLOPEEZ=slopeEZ                 & 
             ,SIGMASFC=sigmasfc,SIGMAEZ=sigmaEZ                 & 
             ,CUPFLAG=cupflag,CLDFRA_CUP=cldfra_cup             & 
             ,CLDFRATEND_CUP=cldfratend_cup                     & 
             ,SHALL=shall,TAUCLOUD=taucloud,TACTIVE=tactive     & 
             ,ACTIVEFRAC=activeFrac                             & 
             ,TSTAR=tstar, LNTERMS=lnterms                      & 
             ,LNINT=lnint                                       & 
             ,NUMBINS=numBins                                   & 
             ,THBINSIZE=thBinSize, RBINSIZE=rBinSize            & 
             ,MINDEEPFREQ=minDeepFreq, MINSHALLOWFREQ=minShallowFreq  & 
             ,WCLOUDBASE=wCloudBase                             & 
             ,WACT_CUP=wact_cup                                 & 
             ,WULCL_CUP=wulcl_cup                               & 
             ,WUP_CUP=wup_cup                                   & 
             ,QC_IC_CUP=qc_ic_cup                               & 
             ,QNDROP_IC_CUP=qndrop_ic_cup                       & 
             ,QC_IU_CUP=qc_iu_cup                               & 
             ,FCVT_QC_TO_PR_CUP=fcvt_qc_to_pr_cup               & 
             ,FCVT_QC_TO_QI_CUP=fcvt_qc_to_qi_cup               & 
             ,FCVT_QI_TO_PR_CUP=fcvt_qi_to_pr_cup               & 
             ,MFUP_CUP=mfup_cup                                 & 
             ,MFUP_ENT_CUP=mfup_ent_cup                         & 
             ,MFDN_CUP=mfdn_cup                                 & 
             ,MFDN_ENT_CUP=mfdn_ent_cup                         & 

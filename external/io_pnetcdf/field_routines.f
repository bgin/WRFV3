...
!local
  ...
  if(Status /= WRF_NO_ERR) then
    write(msg,*) 'NetCDF error in ',"field_routines.F90",', line', 71
...
!local
  ...
  call netcdf_err(stat,Status)
  if(Status /= WRF_NO_ERR) then
    write(msg,*) 'NetCDF error in ',"field_routines.F90",', line', 112
    call wrf_debug ( WARN , msg)
  endif
  return
...
!local
  ...
  call netcdf_err(stat,Status)
  if(Status /= WRF_NO_ERR) then
    write(msg,*) 'NetCDF error in ',"field_routines.F90",', line', 153
    call wrf_debug ( WARN , msg)
  endif
  return
...
!local
  ...
  if(stat/= 0) then
    ...
    write(msg,*) 'Fatal ALLOCATION ERROR in ',"field_routines.F90",', line', 184
  ...
  call netcdf_err(stat,Status)
  if(Status /= WRF_NO_ERR) then
    write(msg,*) 'NetCDF error in ',"field_routines.F90",', line', 215
    call wrf_debug ( WARN , msg)
    return
  endif
  ...
  if(stat/= 0) then
    ...
    write(msg,*) 'Fatal DEALLOCATION ERROR in ',"field_routines.F90",', line', 222
    call wrf_debug ( FATAL , msg)
    return
  endif
  return

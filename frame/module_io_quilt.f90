...
mess )
  ...
  INTEGER, PARAMETER ::  int_noop 			=  	    550
      ...
      CALL MPI_TYPE_SIZE( MPI_INTEGER, itypesize, ierr )
      IF ( itypesize <= 0 ) THEN
        CALL wrf_error_fatal3("<stdin>",1461,&
"external/RSL/module_dm.F: quilt: type size <= 0 invalid")
      ENDIF
...
integer , DIMENSION(max_domains) :: afwa_hailcast_opt
...
NAMELIST /afwa/ afwa_hailcast_opt
...
subroutine wrf_quilt_find_server(iserver)
  ...
  if(in_avail) then
     ...
     if(ierr/=0) then
        call wrf_error_fatal3("<stdin>",8442,&
...
'mpi_barrier failed in wrf_quilt_server_ready')
  ...
  if(in_avail) then
     ...
     if(ierr/=0) then
        call wrf_error_fatal3("<stdin>",8491,&

...
IF ( ASSOCIATED( grid%qv_largescale_tend ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",3103,&
...
IF ( ASSOCIATED( grid%ql_largescale ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",3110,&
...
IF ( ASSOCIATED( grid%tau_x ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",3166,&
...
IF ( ASSOCIATED( grid%q_soil_forcing_tend ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",3215,&
...
IF ( ASSOCIATED( grid%tau_soil ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",3222,&
...
IF ( ASSOCIATED( grid%dfi_scalar_bys ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",3970,&
...
IF ( ASSOCIATED( grid%rushten ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",7386,&
...
IF ( ASSOCIATED( grid%rucuten ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",7463,&
...
IF ( ASSOCIATED( grid%rqccuten ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",7498,&
...
IF ( ASSOCIATED( grid%i_rainc ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",7561,&
...
IF ( ASSOCIATED( grid%tracer_btxe ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",15273,&
   ...
   END SUBROUTINE init_module_domain
      ...
      FUNCTION domain_get_start_time ( grid ) RESULT ( start_time ) 
        ...
        CALL WRFU_ClockGet( grid%domain_clock, StartTime=start_time, &
                            rc=rc )
        IF ( rc /= WRFU_SUCCESS ) THEN
          CALL wrf_error_fatal3("<stdin>",15417,&
   ...
   SUBROUTINE get_dims_from_grid_id (  id   &
    ...
    INTEGER, DIMENSION(3), INTENT(INOUT) ::                   &
     ...
     ELSE
        ...
        CALL wrf_error_fatal3("<stdin>",16575,&
TRIM(mess))
     ENDIF
